...
session_list = ['C:/data/LO032/20220909_144008',
...
for session_dir in session_list:  
    ...
    find_splits = False #Tries to find a split, for which both, the training and 
    ...
    # include_regressor.append([0,1,2,3])
     ...
     #Include the full model
    include_regressor = [np.arange(x_all.shape[1]+4)]
    #Include all the "cognitive regressors"
    for k in range(x_all.shape[1]):
        include_regressor.append([k]) #The single variable models
    #Head direction
...
#     lower_lim = (int(xymin/binwidth)) * binwidth
...
# outcome_on = int((1*frame_rate) + (1*frame_rate) + (0.2*frame_rate + 0.3*frame_rate))
# init_on = 0

# tmp_av = []
# tmp_std = []
# # up_to = 0.5 * frame_rate
# up_to = 0
# #start_at = 0
# step_size = 10

# #start_at = outcome_on + 20
# for n in np.arange(0, len(X), step_size):
#     tmp_av = []
#     tmp_std = []
#     start_at = n
#     for k in range(up_to+1):
#         tmp_av.append(np.mean(Y[start_at], axis=0))
#         tmp_std.append(np.std(Y[start_at], axis=0))
    
#     if len(tmp_av) == 1:
#         pop_mean = tmp_av[0]
#         pop_std = tmp_std[0]
#     else:
#         pop_mean = np.mean(tmp_av, axis=0)
#         pop_std = np.mean(tmp_std, axis=0)
...
#     scatter_hist(pop_mean, pop_std, ax, ax_histx, ax_histy)
    
#     ax.set_xlabel('Average over trials')
#     ax.set_ylabel('Std over trials')
#     ax_histx.set_title(f'{n/frame_rate} seconds into the trial')

#%%
# mean_thresh = 0
# std_thresh = 1

# keep_neuron = np.where((pop_mean > mean_thresh) & (pop_std > std_thresh))[0]
    ...
    if cv_mode == 'manual':
        ...
        #in the folds is closest to the mean of the entire data
            ...
            for time_t in range(len(Y)):
                ...
                all_testing = []
                abs_deviance = []
                for k in range(10**3): #10**3 splits seems to yield good results for the splitting
            ...
            #---------
            encoding_models = pd.DataFrame(index = np.arange(len(include_regressor)), columns = ['models', 'include_regressor', 'training_splits', 'testing_splits',])
