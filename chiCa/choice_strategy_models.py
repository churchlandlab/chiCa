...
#-----------------------------------------------------------------------------
#%%------ Fit Gaussian mixture to cluster data -----
# This is adapted from the original creategaussian by Abbie Yu to identify
# the approach side of the mice

def fit_gaussian_mixture(x_train, x_predict, number_of_gaussians = 2):
    
    '''Function to create guassian distributions from one matrix with ground truth and apply predictions to second matrix of interest
     
    Parameters
    ----------
    x_train: matrix you want to train the model on 
    x_predict: matrix you want to run the predictions on 
    number_of_gaussians: int, number of gaussian components to fit to the data
    ...
    gm_model=GaussianMixture(n_components=2).fit(x_train)
    ...
    # gm_probabilities=gm_x_matrix.predict_proba(x_matrix_predict)
