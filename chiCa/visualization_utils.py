...
def fancy_violin(axis, data, violin_colors=None, labels = None, x_label = None, y_label = None, widths = 0.5):
    ...
    ax.scatter(inds, medians, marker='o', color='white', edgecolors = gray, s=30, zorder=3)
...
#-----------------------------------------------------------------------------
#%%
def fancy_boxplot(axis, data, box_colors, labels = None, x_label = None, y_label = None, widths = 0.5):
    ...
    bp = ax.boxplot(data, labels=labels, widths = widths, showmeans=True, showfliers = False, patch_artist=True, medianprops = medianprops, meanprops = meanprops)
...
def box_scatter(axis, box_data, labels = None, bar_colors = None, x_label = None, y_label = None, scatter_size = 20):
    ...
    ax.axhline([0], color = 'k', linewidth = ax.spines['top'].get_linewidth())
    ax.bar(np.arange(1,box_data.shape[1]+1), np.mean(box_data, axis=0), color = bar_colors, edgecolor = 'k', linewidth = 1, yerr = box_error, ecolor = 'k', capsize = 3, error_kw = {'linewidth': 1})
    jitter = (np.random.rand(box_data.shape[0]) - 0.5) * 0.5
    for k in range(1, box_data.shape[1]+1):
        ax.scatter(k + jitter, box_data[:,k-1], scatter_size, c = 'w', linewidths = 1, edgecolors = gray, zorder = 2)
    ax.set_xlabel(x_label)
    ax.set_ylabel(y_label)
    if labels is not None:
        ax.set_xticks(np.arange(1, len(labels) + 1))
...
#------------------------------------------------------------------------
#%%
def fancy_scatter(axis, x_data, y_data, color = None, scatter_size = 20, x_label = None, y_label = None):
    '''Draw scatter plot with thin gray scatter outline.'''
    gray = '#c1cdcd'
    ax.scatter(x_data, y_data, s = scatter_size, c = color, edgecolor = gray, linewidth=0.3)
    ax.set_xlabel(x_label)
    ax.set_ylabel(y_label)
...
def plot_timecourse(ax, data, frame_rate, index_list, spacer = 8, colors = None, linestyles = '-', vline_idx = None, line_labels = None,
    ...
    for k in range(len(data)):
       for q in range(len(idx_list)):
           if q == 0:
               x_vect = (idx_list[q] + q*spacer) / frame_rate
           else:
               x_vect = np.arange(x_vect[-1]*frame_rate + spacer, x_vect[-1]*frame_rate + spacer + len(idx_list[q])) /frame_rate

           if data[k].shape[1] > 1:
               av = np.mean(data[k][idx_list[q],:],axis=1)
               sem = np.std(data[k][idx_list[q],:],axis=1)/np.sqrt(data[k].shape[1])
           else:
               av = np.squeeze(data[k][idx_list[q],:])
               sem = np.zeros([av.shape[0]])*np.nan
               
           ax.fill_between(x_vect, av - sem, av + sem, color = colors[k], alpha = 0.6, edgecolor = None)
