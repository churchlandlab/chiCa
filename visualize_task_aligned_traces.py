...
def visualization_tool(signal, trialdata, average_interval, window):
    '''Visualization tool to plot task aligned traces.'''
    ...
    def determine_prior_variable(tracked_variable):
        ...
        for n in range(tracked_variable.shape[0]):
            if np.isnan(tracked_variable[n]) == 0:
                prior_label[n] = retain_val
                retain_val = tracked_variable[n]
    ...
    def figure_updater(fig_handle, data_axes, lines_on_plots, signal, trialdata, average_interval, window):
        ...
        for m in range(5):
            ...
            lines_on_plots[10+m] = plot_signal(data_axes[10+m], lines_on_plots[10+m], aligned_signal, x_vect, prior_choice,['Priot right choice', 'Prior left choice'])
        ...
        for m in range(len(consider_states)):
            data_axes[m].set_title(consider_states[m])
...
visualization_tool(signal, trialdata, average_interval, window)
